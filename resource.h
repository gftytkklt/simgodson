...
struct res_desc {
  ...
  unsigned int alloc_stamp;  /* time last allocated */
  struct res_template {
    ...
    int fu_index;           		/* function unit index this belongs to */
    int busy;           		/* for non-pipeline sub-units:div */
...
extern void res_dump(struct res_pool *pool, FILE *stream);

//extern void resource_init(void);
//extern void release_fu(void);
//extern struct res_template * my_res_get(int class);

/* allocation proper function unit for op with given class */
//extern struct res_template * res_alloc ( int class);
/* op is to enter integer queue ? */
extern int is_intq_op(int op);
/* total function unit number */
//extern int res_get_fu_number(void);
