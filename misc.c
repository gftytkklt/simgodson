...
char *
advance_string(char *s, char key, int num)
{
  int i;

  s++; 				/* Bypass first key, if the string starts with it */
  while (*s != key) s++;	/* Find the next key */
  for (i=1; i<num; i++)		
  {
    s++;
    while (*s != key) s++;	/* Find the next key */
...
void 
process_resources(char *config_string, 
		  char *names[], 
		  int num_resources, 
		  int num_args)
{
  ...
  config_string = advance_string(config_string, ':', num_args);
  for (j=0; j<num_resources; j++) 
    {
      s = names[j] = (char *) malloc(sizeof(char)*128);
      config_string++;
      while((*config_string != '\0') && (*config_string != ':'))
	*s++ = *config_string++;
      *s = '\0';
      if (*names[j] == '\0')
        fatal("bad resource name");
    }
}
