...
void calculate_time (result_type *result,arearesult_type *arearesult,area_type *arearesult_subbanked,parameter_type *parameters, double *NSubbanks, double Nspd_predef, int Ndwl_predef, int Ndbl_predef, int optimize)
{
  ...
  int Ndwl_lowlimit, Ndwl_uplimit, Ndbl_lowlimit, Ndbl_uplimit;
  double Nspd;
  double Nspd_lowlimit, Nspd_uplimit;
  ...
  // added by wangjun
  if (optimize == 0) {
    Nspd_lowlimit = Nspd_predef;
    Nspd_uplimit = Nspd_predef;
    Ndwl_lowlimit = Ndwl_predef;
    Ndwl_uplimit = Ndwl_predef;
    Ndbl_lowlimit = Ndbl_predef;
    Ndbl_uplimit = Ndbl_predef;
  } else {
    Nspd_lowlimit = (double)(BITOUT)/(double)(parameters->block_size*8);
    Nspd_uplimit = MAXDATASPD;
    Ndwl_lowlimit = 1;
    Ndwl_uplimit = MAXDATAN;
    Ndbl_lowlimit = 1;
    Ndbl_uplimit = MAXDATAN;
  }
  ...
  for (counter = 0; counter < 2; counter++)
  {
    if (!parameters->fully_assoc)
    {
      ...
      for (Nspd = Nspd_lowlimit; Nspd <= Nspd_uplimit; Nspd = Nspd * 2) 
      {
        /* 
        ...
        for (Ndwl = Ndwl_lowlimit; Ndwl <= Ndwl_uplimit; Ndwl = Ndwl * 2) 
        {
          /* 
          ...
          for (Ndbl = Ndbl_lowlimit; Ndbl <= Ndbl_uplimit; Ndbl = Ndbl * 2) 
          {
            /*
